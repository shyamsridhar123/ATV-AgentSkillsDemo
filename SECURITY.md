...
- **Reachability:** The advisory only affects `v3`/`v5`/`v6` when the caller passes a `buf` argument. `@azure/msal-node` only calls `uuid.v4()`, so this codebase was not actually exploitable — the override exists to clear the audit signal, not to patch a live exposure.
